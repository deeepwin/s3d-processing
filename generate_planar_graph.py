...
# WMART - create colormap to draw connections in different colors
import random
N = 200
colors = []
for i in range(N):
...
def generate_graph(annot, image_path, out_path):
    ...
    # WMART - dont't output numpy annotation for the moment
    ...
    for c, connections in global_graph.items():
        for other_c in connections:
            nb_of_connections += 1
            cv2.line(viz_image, (int(c[0]), int(c[1])), (int(other_c[0]), int(other_c[1])), color=colors[nb_of_connections], thickness=2)
    
    print('Total connections for {} {}'.format(out_path.split(os.path.sep)[-1], nb_of_connections))
    ...
    #     room_idx += 1
    cv2.imwrite(os.path.join(out_path, 'planar_graph_walls.png'), viz_image)
...
# WMART - update path to test, val or train dataset
data_base = '/mnt/data/github/s3d-processing/data/montefloor_data/test/'
